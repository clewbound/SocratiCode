...
* **Fast-path indexing — same-collection skip and sibling-collection clone.** `codebase_index` now consults `git ls-files -s` (via `getGitBlobShas`) to compare working-tree blob shas against stored project metadata before scanning. (1) **Same-collection skip:** if the stored `gitBlobShas` matches the current tree exactly, scan + embed are skipped and only the code graph is refreshed (~5 s on a 26k-file repo). (2) **Sibling-collection clone:** when a target collection is empty but a sibling collection (same project path, different collection name) has overlapping shas, points are cloned via `cloneCollectionPoints`, the diff is computed via `diffGitTrees`, stale chunks are deleted, and only the modified + added subset is re-scanned. Typical feature-branch checkout (~10 modified files on a 26k-file repo) completes in ~30-45 s. Falls through to a full index on missing `.git`, missing sibling, partial clone failure, or active merge conflicts. See README § "Fast-path indexing".
