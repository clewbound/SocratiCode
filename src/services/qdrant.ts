 ...
 */
export const METADATA_COLLECTION = `${QDRANT_COLLECTION_PREFIX}socraticode_metadata`;
...
// ── Symbol cache collection ──────────────────────────────────────────────

const SYMBOL_CACHE_COLLECTION = "socraticode_symbol_cache";

/** Cached flag: once the symbol cache collection is confirmed to exist, skip re-checking */
let symbolCacheCollectionReady = false;

/** Reset the symbol cache collection readiness cache (for testing only) */
export function resetSymbolCacheCollectionCache(): void {
  symbolCacheCollectionReady = false;
}

/** Ensure the symbol cache collection exists (idempotent, cached after first success).
 *  The collection stores the per-file output of the symbol extraction pipeline
 *  ({symbols, rawCalls, imports}) keyed by `(SCHEMA_VERSION, language, gitBlobSha)`.
 *  It is never searched, so it uses a 1-dim dummy vector (Qdrant requires vectors).
 *  Handles concurrent creation: the graph builder calls this in parallel
 *  via Promise.all, so multiple callers can race past the existence check. We swallow
 *  the resulting "Conflict" / "already exists" error since either outcome leaves the
 *  collection in the same usable state. */
export async function ensureSymbolCacheCollection(): Promise<void> {
  if (symbolCacheCollectionReady) return;

  const qdrant = getClient();
  const collections = await qdrant.getCollections();
  const exists = collections.collections.some((c) => c.name === SYMBOL_CACHE_COLLECTION);
  if (!exists) {
    try {
      await qdrant.createCollection(SYMBOL_CACHE_COLLECTION, {
        vectors: { size: 1, distance: "Cosine" },
        on_disk_payload: true,
      });
      logger.info("Created symbol cache collection");
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      if (!/already exists|conflict/i.test(msg)) throw err;
      // Another concurrent caller created it — that's fine.
  ...
  symbolCacheCollectionReady = true;
}
 ...
 *  Candidates are codebase collections sharing the projectId prefix (path-hash
 *  for legacy, repoId for daemon mode); this includes branch-aware suffixed
 *  variants (`codebase_<prefix>__<branch>`) plus the bare branch-unaware
...
export async function findSiblingMetadata(
  ...
  // Sibling discovery uses the current projectId's prefix (the part before
  // any `__<branch>` suffix). This works for:
  //   - legacy path-keyed collections (`<pathhash>` or `<pathhash>__<branch>`)
  //   - repo-keyed collections under daemon mode (`<repoId>__<branch>`)
  // The prefix derivation is a no-op when SOCRATICODE_REPO_KEYING is off.
  const projectId = projectIdFromPath(projectPath);
  const prefixId = projectId.split("__")[0];
  const baseName = `codebase_${prefixId}`;
