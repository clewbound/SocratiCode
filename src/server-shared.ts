 ...
 * Used by both the stdio entrypoint (default) and the daemon HTTP entrypoint.
 *
 * Tool registrations are kept verbatim from the legacy inline construction in
 * `src/index.ts` — only the enclosing scope changed. Any change to a tool
 * description, schema, or handler should be made here and is automatically
 * picked up by both transports.
 */
